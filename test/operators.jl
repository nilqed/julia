...
@testset "function composition" begin
    @test (Base.UTF8proc.uppercase∘hex)(239487) == "3A77F"
end
@testset "function negation" begin
    str = randstring(20)
    @test filter(!Base.UTF8proc.isupper, str) == replace(str, r"[A-Z]", "")
    @test filter(!Base.UTF8proc.islower, str) == replace(str, r"[a-z]", "")
end
